...
- Downloaded hallmark(H) , KEGG and GO gene sets from [ MSigDB website](https://www.gsea-msigdb.org/gsea/msigdb/human/collections.jsp#H).
...
# 3. Conduct analysis
# Run fgsea and fgseaMultilevel with 'pathwaysH' and 'ranks'.
fgseaRes <- fgsea(pathwaysH, ranks)
fgseaRes <- fgseaMultilevel(pathways=pathwaysH, stats=ranks)
...
write.table(fgseaResTidy, "~/BHK lab/kevin Project/Cholangiocarcinoma/output data/fgseaRes_HRanks.txt", sep = "\t", row.names = FALSE, quote = FALSE)
...
# For significant pathways (padj < 0.05).

plotEnrichment(pathwaysH[["HALLMARK_INTERFERON_ALPHA_RESPONSE"]], ranks)
