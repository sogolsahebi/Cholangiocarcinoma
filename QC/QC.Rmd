...
expr_tpm <- expr_tpm[-remove, ]   #dim is 6789 x 33

dim(expr_tpm)

# transpose the expr_tpm for later use in pCA.
#expr_tpm <- t(expr_tpm)
...
- **Continuous Variables (e.g., Age):** We calculated the median and IQR and assessed associations using the Wilcoxon test. 
  - _Reason:_ The Wilcoxon test is used because it's non-parametric and effective for data not normally distributed.
...
- **Discrete/Categorical Variables (e.g., Sex, Stage, Location):** We determined frequency and percentage distributions and assessed associations using Fisher's exact test.
  - _Reason:_ Fisher's exact test is preferred over chi-squared in small samples because it's more accurate with low frequencies (less than 5).
...
# TODO: Convert "IIIA", "IIIB", "IIIC" to "IIIA and convert "IVA", "IVB" to "IV".
...
# Assuming summary_stats, sex_stats, location_stats, stage_stats, and p_values are already defined
...
# Differential gene expression.
  ...
  - **Volcano plot and csv file **
  

```{r diff expression }

# 1. Limma approach applied.
...
# Preview
DT::datatable(topTable(fit))

# Write the data frame to a CSV file
file_path <- "~/BHK lab/kevin Project/Cholangiocarcinoma/data/Limma_Fit.csv"
write.csv(data.frame(RowNames = rownames(fit),fit), file_path, row.names = FALSE)
